...
def forward_kinematics(q_rad, L=LINK_LENGTHS):
    ...
    # r-z plane coordinates
    r_j2 = L1 * cos(t2)
    z_j2 = L1 * sin(t2)
    r_j3 = r_j2 + L2 * cos(t2 + t3)
    z_j3 = z_j2 + L2 * sin(t2 + t3)
    r_tip = r_j3 + L3 * cos(t2 + t3 + t4)
    z_tip = z_j3 + L3 * sin(t2 + t3 + t4)
    # 3D coordinates
    p_base = np.array([0.0, 0.0, 0.0])
    p_elbow = np.array([r_j2 * cos(t1), r_j2 * sin(t1), z_j2])
    p_wrist = np.array([r_j3 * cos(t1), r_j3 * sin(t1), z_j3])
    p_tip = np.array([r_tip * cos(t1), r_tip * sin(t1), z_tip])
    return [p_base, p_base, p_elbow, p_wrist, p_tip]
